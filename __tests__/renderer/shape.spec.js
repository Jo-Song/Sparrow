...
describe('shapes', () => {
  test('shape(name, context, attribute) create SVG elements with specified attribute and mounts it to group.', () => {
    const renderer = createRenderer(600, 400);
    const context = { group: renderer.group() };

    const s = shape('circle', context, {
      cx: 100, cy: 100, r: 50, fill: 'red', stroke: 'yellow', strokeWidth: 10,
    });
    ...
    expect(getAttributes(s, ['cx', 'cy', 'r', 'fill', 'stroke', 'stroke-width'])).toEqual({
      cx: '100', cy: '100', r: '50', fill: 'red', stroke: 'yellow', 'stroke-width': '10',
    });
  });
  ...
  test('line() creates line element.', () => {
    ...
    const line = renderer.line({
      x1: 0, y1: 0, x2: 50, y2: 50, stroke: 'black',
    });
    mount(createDiv(), renderer.node());
    expect(line.tagName).toBe('line');
  });

  test('text() create text element and sets textContent.', () => {
    const renderer = createRenderer(600, 400);
  ...
  test('path() create path element and accepts array to specify path.', () => {
    const renderer = createRenderer(600, 400);
    const d = [
      ['M', 10, 10], ['L', 100, 100], ['L', 100, 10], ['Z'],
    ];
    const path = renderer.path({ d, stroke: 'black', fill: 'red' });
  ...
  test('ring() creates three circle elements to simulate a ring.', () => {
    ...
    const ring = renderer.ring({
      cx: 100, cy: 100, r1: 30, r2: 60, strokeWidth: 10, stroke: 'red', fill: 'blue',
    });
    mount(createDiv(), renderer.node());
    const [c0, c1, c2] = ring;
